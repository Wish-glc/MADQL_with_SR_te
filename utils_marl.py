...
#     Yb =  np.zeros((PSID.shape[0], 1), dtype=np.float32)  # 使用 float32 节省内存
#     B = np.zeros((PSID.shape[0],flows.shape[0]), dtype=np.float32)  # 使用 float32 节省内存
...
#         # Optimized: PSID[p] corresponds to (i, j) where p = i*N + j
#         contatore = int(flows[f,1] * N + flows[f,2])
#         B[contatore, f] = 1
...
#--------------修改后-----------------------#
def startingPSID(SL, G, flows, N, linkRete):
    PSID = np.zeros((N*N,3), dtype=int)
    Yb = np.zeros((PSID.shape[0], 1), dtype=np.float32)

    # 🔥 关键修改：稀疏矩阵
    B = lil_matrix((PSID.shape[0], flows.shape[0]), dtype=np.float32)
...
def compute_LinkRete(max_matrix_blu):
    ...
    R = np.zeros((L,K), dtype=np.float32)  # 使用 float32 节省内存
...
def multi_aggiornaPSID(SL, cammini, flows, N, linkRete, dict_encap_dict):
    ...
    for f in range(len(flows)): # for each flow
        ...
        # Optimized: PSID[p] corresponds to (i, j) where p = i*N + j, so we can calculate directly
        psid = int(flows[f,1] * N + flows[f,2]) #-> identify the psid of the base flow (optimized from array search)
        ...
        #iterating over the a link i.e. 2 consecutive element in a complex path and so 2 touples of (node, active secment)
            ...
            # Optimized: PSID[p] corresponds to (i, j) where p = i*N + j
            id_contatore = int(node_e * N + act_seg_s)
        ...
        for a in range(1,len(sl)):
            ...
            for n in  range(len(tmp)-1):
                ...
                if tmp[n] != id_agent or (tmp[n] == id_agent and sl[a] not in dict_encap ):
                    ...
                    # Optimized: PSID[p] corresponds to (i, j) where p = i*N + j
                    id_contatore = int(tmp[n + 1] * N + sl[a])
                    ...
                    for a_new in range(1,len(sl_new)):
                        ...
                        for n in  range(len(tmp)-1):
                            ...
                            # Optimized: PSID[p] corresponds to (i, j) where p = i*N + j
                            id_contatore = int(tmp[n+1] * N + sl_new[a_new])
