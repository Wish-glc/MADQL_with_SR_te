...
if __name__ == "__main__":
    ...
    if train_flag == 'train': #train
        ...
        for i_game_score in range(i_game_score+1, len(index_train)):
            ...
            #-------------修改添加释放内存-----------------#
            #清理大数组以释放内存
            del flows_blu, R_blu, SL_blu, YbExp_blu, PSID_blu, cammini_blu
            del flows_orange, R_orange, SL_orange, YbExp_orange, PSID_orange, cammini_orange
            del TM_blu, TM_orange
            # 定期强制垃圾回收
            if i_game_score % 10 == 0:
                gc.collect()
        ...
        #for i_game_score in range(1):
            ...
            for R, flows in zip([R_orange], [flows_orange]):
                ...
                #--------------------修改-------------------#
                # request = R.dot(flows[:,-1]) 原始代码
                #--------------------修改后-------------------#
                # # 使用稀疏矩阵乘法优化性能
                request = R @ flows[:, -1]
                request = np.asarray(request).ravel()
